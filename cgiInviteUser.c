...
// Description: cgiInviteUser() primary function source file 
// Date: 14/02/2005
//=================================================================//
...
static const char rcsid [] = "$Id: cgiInviteUser.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (YOU_CAN_ONLY_INVITE_USERS_BY_YOURSELF);
    exit (ML_CGI_OK);
  }
  
  if ((errorCode = mlCgiGetFormStringNoNewLines ("name", name, MAX_LENGTH_NAME)) != ML_CGI_OK)
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    if (errorCode == ML_CGI_FORM_FIELD_NOT_FOUND)
  ...
  if ((errorCode = inviteUser (givenId, name, email)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
