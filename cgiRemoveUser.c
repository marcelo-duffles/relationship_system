...
// Description: cgiRemoveUser() primary function source file 
// Date: 13/02/2005
//=================================================================//
...
static const char rcsid [] = "$Id: cgiRemoveUser.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (ONLY_THE_ADMINISTRATOR_CAN_REMOVE_OTHERS_USERS);
    exit (ML_CGI_OK);
  }
  ...
  if ((errorCode = removeUser (givenId)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
