...
// Description: cgiAddParentRelationship() primary function source file 
// Date: 14/02/2005
//=================================================================//
 ...
 * $Log: cgiAddParentRelationship.c,v $
...
static const char rcsid [] = "$Id: cgiAddParentRelationship.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId) && (givenId2 != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (YOU_CAN_ONLY_ADD_PARENTS_FOR_YOU);
    exit (ML_CGI_OK);
  }
  ...
  if ((errorCode = addParentRelationship (givenId, givenId2)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
