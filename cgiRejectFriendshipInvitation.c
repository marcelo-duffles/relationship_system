...
// Description: cgiRejectFriendshipInvitation() primary function source file 
// Date: 14/02/2005
//=================================================================//
 ...
 * $Log: cgiRejectFriendshipInvitation.c,v $
...
static const char rcsid [] = "$Id: cgiRejectFriendshipInvitation.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (YOU_CAN_ONLY_REJECT_YOURS_INVITATIONS);
    exit (ML_CGI_OK);
  }
  ...
  if ((errorCode = rejectFriendshipInvitation (givenId, givenId2)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
