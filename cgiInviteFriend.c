...
// Description: cgiInviteFriend() primary function source file 
// Date: 14/02/2005
//=================================================================//
...
static const char rcsid [] = "$Id: cgiInviteFriend.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (YOU_CAN_ONLY_INVITE_FRIENDS_BY_YOURSELF);
    exit (ML_CGI_OK);
  }
  ...
  if ((errorCode = inviteFriend (givenId, givenId2)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
