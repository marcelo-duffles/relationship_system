...
// Description: cgiChangePassword() primary function source file 
// Date: 14/02/2005
//=================================================================//
...
static const char rcsid [] = "$Id: cgiChangePassword.c,v 1.1 2005/02/16 22:11:21 marceloddm Exp marceloddm $";
...
int main (int argc, char **argv)
{
  ...
  if ((callerOfProgramId != 0) && (givenId != callerOfProgramId))
  {
    mlCgiEndHttpHeader ();
    mlCgiFreeResources ();
    showHtmlErrorPage (ONLY_THE_ADMINISTRATOR_CAN_CHANGE_PASSWORD_OF_OTHERS_USERS);
    exit (ML_CGI_OK);
  }
  ...
  if ((errorCode = changePassword (givenId, password)) != OK)
    showHtmlErrorPage (errorCode);
  else
    showHtmlOKPage ();    
