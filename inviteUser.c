  ...
  if (( retCode = putString ( writeFile, ENCRYPTED_PASS_LENGTH, ENCRYPTED_PASS_LENGTH, encryptedPass) ) != OK )
  {
    ...
    snprintf (emailBody, 500, "\n\
      *** Essa é uma mensagem de sistema, você não precisa respondê-la! ***\n\
      ...
      Caro(a) %s,\n\
      \n\
        %s está te convidando para entrar no nosso sistema.\n\
      \n\
        Visite o endereço http://www2.del.ufrj.br/~marceloddm/computacao_ii/pf/cgi/showNewUserLoginForm.cgi e responda o convite.\n\
      \n\
        Sua senha provisória: %s\n\
      \n\
      \n\
